function [pmat_td, pmat_gr, reest] = TM_hierarchical(sbj_id, mode, beta, delta, epsilon, gamma, datdir)
    ...
    for t = 1:ntrl
        if t < ntrl
            ...
            if resp_g(t) == 4
                ...
                n_p_gr_ud = zeros(1,length(gr_cand));
                for i = 1:length(gr_cand)
                    if ismember(td_cand(i),map_td)
                        n_p_gr_ud(i) = p_gr(i)*((1-gamma)*beta+gamma*(1-beta));
                    elseif td_cand(i)~=0
                        n_p_gr_ud(i) = p_gr(i)*((1-gamma)*(1-beta)+gamma*beta);
                    else
                        n_p_gr_ud(i) = p_gr(i)*gamma;
                    end
                end
                n_p_gr_ud = n_p_gr_ud/sum(n_p_gr_ud);

                if t == 1 || ismember(tigroar_g(t),pred_td) % predicted TD = observed tiger roar : UPDATING
                    n_p_gr = n_p_gr_ud;

                else % predicted TD != observed tiger roar : RE-ESTIMATION
                     n_p_ge_re = zeros(1,length(gr_cand));
                     for i = 1:length(gr_cand)
                        if ismember(td_cand(i),map_td)
                            n_p_ge_re(i) = p_gr_ts(i)*((1-gamma)*beta+gamma*(1-beta));
                        elseif td_cand(i)~=0
                            n_p_ge_re(i) = p_gr_ts(i)*((1-gamma)*(1-beta)+gamma*beta);
                        else
                            n_p_ge_re(i) = p_gr_ts(i)*gamma;
                        end
                     end
                     n_p_ge_re = n_p_ge_re/sum(n_p_ge_re);
                        
                     n_p_gr = epsilon*n_p_gr_ud + (1-epsilon)*n_p_ge_re;
